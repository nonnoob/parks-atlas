/* Parks Atlas — main app (national overview + state view + gesture scratch).
   Data: data.js · Strings: i18n.js · Config: config.js · Crypto ledger: ledger.js */
(function(){
"use strict";
const PARKS=window.PARKS, REGIONS=window.REGIONS, TIERS=window.TIERS, STATES=window.STATES, PSF=window.PARK_STATE_FALLBACK||{};
const T=window.I18N, t=T.t;
const $=s=>document.querySelector(s);
const el=(t2,c)=>{const e=document.createElement(t2);if(c)e.className=c;return e;};
...
/* ---------- 云存档（Cloudflare Worker 中转，docs/self-hosting.md + ADR-0010） ----------
   打卡后自动把加密存档推到 GitHub 仓库；任何设备输口令即恢复。CLOUD 为空则整套静默关闭。
   Fork 者：CLOUD 在 config.js 里改，别改这里 */
const CLOUD=(window.ATLAS_CONFIG||{}).CLOUD||'';
...
async function cloudPush(){
  ...
  try{
    ...
    if(r.status===409){localStorage.removeItem(dirtyKey());toast(t('cloudBoundPush'));return;}
    if(!r.ok)throw new Error(r.status);
    localStorage.removeItem(dirtyKey());
  }catch(e){localStorage.setItem(dirtyKey(),'1');toast(t('cloudFail'));}
}
...
async function cloudSyncAfterUnlock(pass){
  ...
  try{
    ...
    if(!m){toast(t('cloudBoundSync'));return;}
    const p=ledger.purgeInvalid();
    if(m.added||p)refreshAll();
    if(m.added)toast(t('cloudSynced',m.added));
...
async function enterWithPass(pass){
  ...
  if(CLOUD){try{const b=await cloudGet(id);if(b){const fp=await ledger.importCloud(pass,b);if(fp){refreshAll();toast(t('restored',fp));return true;}return false;}}catch(e){}}
  return 'new';
}
async function createAtlas(pass){await ledger.setup(pass);refreshAll();schedulePush();}
/* Promise 弹窗（口令/确认/日期）在语言切换时原地重渲染而不是关闭——关了 Promise 就悬挂了。
   msgKey 传 i18n 键而非成品文案，重渲染才能换语言 */
function askGate(msgKey){
  return new Promise(resolve=>{
    let val='';
    const done=v=>{MODAL_RELANG=null;MODAL_ONCLOSE=null;closeModal();resolve(v);};
    const render=()=>{
      openModal('<h3 style="display:flex;justify-content:space-between;align-items:center">'+t('gateTitle')+'<a id="glang" style="cursor:pointer;font-size:12px;font-weight:700">🌐 '+t('langBtn')+'</a></h3>'+(msgKey?'<p>'+t(msgKey)+'</p>':'')+'<input class="inp" type="password" id="gp" autocomplete="off"><div class="mbtns"><button id="gc">'+t('cancel')+'</button><button class="pri" id="go">'+t('enter')+'</button></div>');
      const inp=$('#gp');inp.value=val;inp.focus();
      inp.oninput=()=>{val=inp.value;};
      $('#glang').onclick=()=>setLang(T.lang==='zh'?'en':'zh');
      $('#gc').onclick=()=>done(null);
      const go=()=>{if(inp.value.length<4)return toast(t('passMin'));done(inp.value);};
      $('#go').onclick=go;inp.onkeydown=e=>{if(e.key==='Enter')go();};
    };
    MODAL_RELANG=render;MODAL_ONCLOSE=()=>resolve(null);render();
  });
}
function confirmNew(){
  return new Promise(resolve=>{
    const done=v=>{MODAL_RELANG=null;MODAL_ONCLOSE=null;closeModal();resolve(v);};
    const render=()=>{
      openModal('<h3>'+t('newPassTitle')+'</h3><p>'+t('newPassBody')+'</p><div class="mbtns"><button id="nc">'+t('back')+'</button><button class="pri" id="no">'+t('newAtlas')+'</button></div>');
      $('#nc').onclick=()=>done(false);
      $('#no').onclick=()=>done(true);
    };
    MODAL_RELANG=render;MODAL_ONCLOSE=()=>resolve(false);render();
  });
}
async function gate(){
  ...
  while(true){
    ...
    msg=r==='new'?null:'gateWrong';pass=null;
...
function renderStatic(){
  ...
  $('#htitle').innerHTML=t('htitle')+'<small id="htsub">'+t('htsub')+'</small>';
...
function setLang(l){
  ...
  renderStatic();renderProgress();renderRegions();renderBanner();
  if(MODAL_RELANG)MODAL_RELANG();else closeModal();
  buildNational();
...
function renderProgress(){const n=visitedCount();$('#cnt').textContent=n;$('#tier').textContent=tierOf(n);$('#fill').style.width=Math.round(n/PARKS.length*100)+'%';const states=new Set();PARKS.forEach(p=>{if(isVisited(p.id)&&p._state)states.add(p._state);});let latest='';PARKS.forEach(p=>{if(isVisited(p.id)){const d=ledger.record(p.id).d;if(d>latest)latest=d;}});$('#subrow').innerHTML='<span>'+t('subLit',Math.round(n/PARKS.length*100))+'</span><span>'+t('subStates',states.size)+'</span>'+(latest?'<span>'+t('subLatest',latest)+'</span>':'');}
function renderRegions(){const host=$('#regions');host.innerHTML='';REGIONS.forEach(rg=>{const tot=PARKS.filter(p=>p.rg===rg).length;const got=PARKS.filter(p=>p.rg===rg&&isVisited(p.id)).length;const c=el('div','chip'+(curRegion===rg?' on':''));c.innerHTML=T.region(rg)+' <b>'+got+'</b>/'+tot;c.onclick=()=>{curRegion=curRegion===rg?null:rg;renderRegions();paintNational();};host.appendChild(c);});}
...
function sfxScratch(){if(!SOUND||!AC)return;const t2=AC.currentTime,len=Math.floor(AC.sampleRate*0.05);const b=AC.createBuffer(1,len,AC.sampleRate),d=b.getChannelData(0);for(let i=0;i<len;i++)d[i]=(Math.random()*2-1)*0.5;const s=AC.createBufferSource();s.buffer=b;const f=AC.createBiquadFilter();f.type='highpass';f.frequency.value=2600;const g=AC.createGain();g.gain.setValueAtTime(0.16,t2);g.gain.exponentialRampToValueAtTime(0.001,t2+0.06);s.connect(f);f.connect(g);g.connect(AC.destination);s.start(t2);}
function sfxDing(){if(!SOUND||!AC)return;const t2=AC.currentTime;[660,990,1320].forEach((fr,i)=>{const o=AC.createOscillator(),g=AC.createGain();o.type='sine';o.frequency.value=fr;const ts=t2+i*0.08;g.gain.setValueAtTime(0.0001,ts);g.gain.exponentialRampToValueAtTime(0.2,ts+0.02);g.gain.exponentialRampToValueAtTime(0.0001,ts+0.5);o.connect(g);g.connect(AC.destination);o.start(ts);o.stop(ts+0.55);});}
...
async function buildNational(){
  const host=$('#mapHost');
  if(!window.d3||!window.topojson)return buildListFallback(t('mapFailComp'));
  ...
  svg.append('text').attr('x',tb.x+tb.w/2).attr('y',tb.y+15).attr('text-anchor','middle').attr('fill','#5d8794').attr('font-size','10').text(t('territories'));
  const terr={virginislands:[tb.x+38,tb.y+50],samoa:[tb.x+92,tb.y+50]};
  // park markers
  const mg=svg.append('g');
  PARKS.forEach(p=>{let xy=p._terr?terr[p.id]:proj([p.lng,p.lat]);if(!xy)return;const t2=mg.append('text').attr('class','pmark').attr('x',xy[0]).attr('y',xy[1]+4).text(p.em).on('click',(ev)=>{ev.stopPropagation();if(p._terr)openInfo(p);else if(p._state)enterState(p._state);});markEls[p.id]=t2.node();});
  paintNational();
}
...
function enterState(name){
  ...
  head.innerHTML='<button class="sv-back">'+t('backNation')+'</button><div class="sv-mascot">'+(st?st.m:'📍')+'</div><div class="sv-title">'+(st?T.state(st):name)+'<small>'+(st?T.stateSub(st):name)+'</small></div><div class="sv-sub">'+t('svCount',ps.filter(p=>isVisited(p.id)).length,ps.length)+'</div>';
  ...
  if(!ps.length){stage.innerHTML='<div class="sv-emptly"><div class="big2">'+(st?st.m:'🗺️')+'</div><div style="margin-top:8px">'+t('noParks',st?T.state(st):name)+'</div><div style="font-size:12px;margin-top:4px">'+t('tryAnother')+'</div></div>';return;}
...
function askDate(){
  return new Promise(resolve=>{
    const td=todayStr();let val=td;
    const done=v=>{MODAL_RELANG=null;MODAL_ONCLOSE=null;closeModal();resolve(v);};
    const render=()=>{
      openModal('<h3>'+t('dateTitle')+'</h3><input class="inp" type="date" id="cd" value="'+val+'" max="'+td+'"><div class="mbtns"><button id="dc">'+t('cancel')+'</button><button class="pri" id="dk">'+t('stampGo')+'</button></div>');
      $('#cd').oninput=()=>{val=$('#cd').value||td;};
      $('#dc').onclick=()=>done(null);
      $('#dk').onclick=()=>done($('#cd').value||td);
    };
    MODAL_RELANG=render;MODAL_ONCLOSE=()=>resolve(null);render();
  });
}
...
async function finishCheck(p){
  ...
  renderProgress();stampAnim();toast(t('litToast',T.park(p)));
}
...
function openInfo(p,fromList){
  ...
  h+='<div class="pk-top"><div class="pk-emoji">'+p.em+'</div><div><div class="pk-h1">'+T.park(p)+'</div><div class="pk-en">'+t('npSuffix',p)+'</div></div></div>';
  h+='<div style="margin-top:8px"><span class="pill state">📍 '+T.parkSt(p)+'</span><span class="pill yr">'+t('est')+p.yr+'</span>';
  if(vis)h+='<span class="pill done">'+t('visitedPill')+rec.d+'</span>';if(tam)h+='<span class="pill tamper">'+t('sigBad')+'</span>';h+='</div>';
  h+='<div class="intro">'+T.parkIntro(p)+'</div><div class="hl">'+t('highlights')+'<b>'+T.parkHl(p)+'</b></div>';
  h+='<div class="stampbox">';
  if(SHARE){h+='<div class="hint">'+t('readonly')+'</div>';}
  else if(tam){h+='<button class="holdbtn danger" id="holdRemove"><span class="prog2"></span><span class="lab">'+t('holdRemoveBad')+'</span></button>';}
  else if(p._terr){ // territory parks check in here (no state view)
    if(vis){h+='<button class="holdbtn danger" id="holdRemove"><span class="prog2"></span><span class="lab">'+t('holdUnstamp')+'</span></button>';}
    else{h+='<button class="holdbtn" id="holdStamp"><span class="prog2"></span><span class="lab">'+t('holdStamp')+'</span></button>';}
  }
  else if(vis){h+='<button class="holdbtn danger" id="holdRemove"><span class="prog2"></span><span class="lab">'+t('holdUnstamp')+'</span></button>';}
  else if(fromList){h+='<button class="holdbtn" id="holdStamp"><span class="prog2"></span><span class="lab">'+t('holdStamp')+'</span></button>';}
  h+='</div>';
...
function showCallout(p){
  ...
  let h='<span class="cx">✕</span><div class="ct"><span class="ce">'+p.em+'</span><div><div class="ch1">'+T.park(p)+'</div><div class="cen">'+t('npSuffixShort',p)+'</div></div></div>';
  h+='<div style="margin-top:6px"><span class="pill state">📍 '+T.parkSt(p)+'</span><span class="pill yr">'+t('est')+p.yr+'</span></div>';
  h+='<div class="cintro">'+T.parkIntro(p)+'</div><div class="chl">✨ '+T.parkHl(p)+'</div><div class="cstat">';
  if(SHARE)h+='<span style="color:var(--muted)">'+t('readonly')+'</span>'+(vis?' · <span style="color:var(--ok)">✓ '+rec.d+'</span>':'');
  else if(tam)h+='<span style="color:var(--bad)">'+t('sigBad')+'</span><button class="holdbtn danger" id="cRemove"><span class="prog2"></span><span class="lab">'+t('holdDelete')+'</span></button>';
  else if(vis)h+='<span style="color:var(--ok)">'+t('litOn')+rec.d+'</span><button class="holdbtn danger" id="cRemove"><span class="prog2"></span><span class="lab">'+t('holdUnstamp')+'</span></button>';
...
function bindHold(btn,onDone){let raf=null,start=0,done=false;const bar=btn.querySelector('.prog2');const DUR=820;const step=ts=>{if(!start)start=ts;const k=Math.min(1,(ts-start)/DUR);bar.style.width=(k*100)+'%';if(k>=1){done=true;cancel();onDone();}else raf=requestAnimationFrame(step);};const begin=e=>{e.preventDefault();done=false;start=0;raf=requestAnimationFrame(step);};const cancel=()=>{if(raf)cancelAnimationFrame(raf);raf=null;if(!done)bar.style.width='0%';};btn.addEventListener('pointerdown',begin);btn.addEventListener('pointerup',cancel);btn.addEventListener('pointerleave',cancel);btn.addEventListener('pointercancel',cancel);}
async function doStamp(p){if(SHARE)return;if(!await ensurePriv())return;const d=await askDate();if(!d)return;await ledger.checkIn(p.id,'',d);schedulePush();paintNational();renderProgress();closeSheet();stampAnim();toast(t('litToast',T.park(p)));}
async function doRemove(p){if(SHARE)return;if(!await ensurePriv())return;ledger.remove(p.id);schedulePush();paintNational();renderProgress();closeSheet();toast(t('unstamped'));if(MODE==='state'&&p._state)enterState(p._state);}
...
/* ---------- modal ---------- */
let MODAL_RELANG=null; /* 语言切换时由持 Promise 的弹窗登记的重渲染钩子 */
let MODAL_ONCLOSE=null; /* 弹窗被外点/间接关闭时兑现 Promise（=取消），否则调用方悬挂 */
function openModal(html){const m=$('#modal');$('#mcard').innerHTML=html;m.classList.add('show');fixModalZoom();return m;}
function closeModal(){const cb=MODAL_ONCLOSE;MODAL_ONCLOSE=null;MODAL_RELANG=null;$('#modal').classList.remove('show');fixModalZoom();if(cb)cb();}
$('#modal').addEventListener('click',e=>{if(e.target.id==='modal')closeModal();});
function toast(msg){const e=$('#toast');e.textContent=msg;e.classList.add('show');clearTimeout(e._t);e._t=setTimeout(()=>e.classList.remove('show'),1800);}
/* ---------- share ---------- */
$('#btnShare').onclick=()=>{
  if(!ledger.hasIdentity)return openModal('<h3>'+t('shareTitle')+'</h3><p>'+t('shareFirst')+'</p><div class="mbtns"><button class="pri" id="ok">'+t('ok')+'</button></div>'),void($('#ok').onclick=closeModal);
  const link=location.origin+location.pathname+'#'+ledger.shareFragment();
  openModal('<h3>'+t('shareTitle')+'</h3><p>'+t('shareBody')+'</p><div class="share-out" id="so">'+link+'</div><div class="mbtns"><button id="c">'+t('close')+'</button><button class="pri" id="cp">'+t('copyLink')+'</button></div>');
  $('#c').onclick=closeModal;$('#cp').onclick=async()=>{try{await navigator.clipboard.writeText(link);toast(t('copied'));}catch(e){const r=document.createRange();r.selectNodeContents($('#so'));const sel=getSelection();sel.removeAllRanges();sel.addRange(r);toast(t('copySel'));}};
};

/* ---------- footer / help ---------- */
document.addEventListener('click',e=>{if(e.target&&e.target.id==='helpL'){openModal('<h3>'+t('helpTitle')+'</h3><p>'+t('helpBody')+'</p><p style="color:#8fb3b0">'+t('helpNote')+'</p><div class="mbtns"><button class="pri" id="ok">'+t('gotIt')+'</button></div>');$('#ok').onclick=closeModal;}});

/* ---------- banner ---------- */
function renderBanner(){const b=$('#banner');b.className='banner';const tamper=PARKS.some(p=>isTamper(p.id));if(SHARE){b.classList.add('show','ro');b.innerHTML=t('roBanner',ledger.fp)+(tamper?t('roTamper'):'');}else if(tamper){b.classList.add('show','tamper');b.textContent=t('tamperBanner');}}
